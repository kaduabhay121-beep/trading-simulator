...
class SimulationState:
    ...
    def __init__(self):
        ...
        self.live_chain_cache={}; self.chart_cache={}; self.market_cache={}; self.market_cache_ts=0; self.angel=AngelOneData()
        self._load_state()
        self._init_history(); self._running=True
        threading.Thread(target=self._tick_loop,daemon=True).start()

    def _db(self):
        con=sqlite3.connect(self.DB_PATH, timeout=10)
        con.execute('CREATE TABLE IF NOT EXISTS simulator_state (id INTEGER PRIMARY KEY CHECK(id=1), data TEXT NOT NULL)')
        return con

    def _save_state(self):
        data=json.dumps({'wallet':self.wallet,'positions':self.positions,'pending_orders':self.pending_orders,'orders':self.orders,'closed_trades':self.closed_trades,'order_counter':self.order_counter}, separators=(',',':'))
        con=self._db()
        try:
            con.execute('INSERT INTO simulator_state(id,data) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET data=excluded.data',(data,))
            con.commit()
        finally:
            con.close()
    ...
    def _load_state(self):
        try:
            con=self._db(); row=con.execute('SELECT data FROM simulator_state WHERE id=1').fetchone(); con.close()
            if not row: return
            d=json.loads(row[0])
            if isinstance(d.get('wallet'),dict): self.wallet.update(d['wallet'])
            self.positions=d.get('positions',[]) or []
            self.pending_orders=d.get('pending_orders',[]) or []
            self.orders=d.get('orders',[]) or []
            self.closed_trades=d.get('closed_trades',[]) or []
            self.order_counter=int(d.get('order_counter',self.order_counter))
        except Exception as e:
            self.angel.last_error=f'State load: {e}'
    ...
    def _internal_exit(self,pos_id,reason='MANUAL'):
        for i,p in enumerate(self.positions):
            if p['id']==pos_id:
                ...
                self.closed_trades.insert(0,{'id':p['id'],'symbol':p['symbol'],'action':p['action'],'qty':p['qty'],'buy_price':p['buy_price'],'exit_price':p['ltp'],'pnl':pnl,'reason':reason,'time':datetime.now(IST).strftime('%H:%M:%S')}); self._save_state(); return
    ...
    def reset(self):
        self.wallet={'initial':100000.0,'balance':100000.0,'used_margin':0.0,'realized_pnl':0.0}; self.positions=[]; self.pending_orders=[]; self.orders=[]; self.closed_trades=[]; self.order_counter=100; self._save_state()
