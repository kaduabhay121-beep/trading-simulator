...
class SimulationState:
    ...
    def _load_state(self):
        try:
            ...
            saved=d.get('bot',{})
            for k in ('strategy','underlying','instrument_mode','qty','risk_per_trade','max_daily_loss','stop_loss_pct','target_pct','trades_today','daily_pnl','last_trade_ts','risk_lock','risk_lock_reason','last_entry_price'):
    ...
    def _bot_option_symbol(self, underlying, action):
        is_s=underlying=='SENSEX'; spot=self.sensex_spot if is_s else self.nifty_spot; step=100 if is_s else 50; expiry=self.get_option_chain(underlying).get('selected_expiry')
        if not expiry: return None
        strike=round(spot/step)*step; typ='CE' if action=='BUY' else 'PE'
        arr=self.angel.option_instruments(underlying,expiry) if self.angel.enabled else []
        if arr:
            x=next((i for i in arr if abs(float(i.get('strike',0))/100-strike)<0.01 and str(i.get('symbol','')).upper().endswith(typ)),None)
            if x: return f"{underlying}_{expiry}_{int(strike)}_{typ}"
        return None
    ...
    def _run_bot_once(self):
        ...
        signal,conf,meta=strategy_signal(candles,b['strategy']); b['last_signal']=signal; b['last_confidence']=conf; b['last_reason']=json.dumps(meta,separators=(',',':'))
        if signal=='HOLD' or conf<0.70: return
        loss_limit=abs(b['max_daily_loss']/100)*b['initial_balance']
        ...
        b['risk_lock']=False; b['risk_lock_reason']=''
        # Paper bot is intentionally long-only. A SELL signal exits an existing bot position; it never opens a short.
        bot_positions=[p for p in self.positions if p.get('bot_tag') and str(p.get('symbol','')).startswith(b['underlying'])]
        if signal=='SELL':
            for p in bot_positions: self._internal_exit(p['id'],'BOT_SIGNAL')
            return
        if len(self.positions)>=3 or bot_positions: return
        ...
        qty=max(1,int(b['qty'])); cost=round(ltp*qty,2)
        if cost>self.wallet['balance']: return
        sl=round(ltp*(1-b['stop_loss_pct']/100),2); target=round(ltp*(1+b['target_pct']/100),2)
        self._execute_fill(symbol,'BUY',qty,ltp,sl,target,0,bot_tag=True)
        b['last_entry_price']=ltp
        b['trades_today']+=1; b['last_trade_ts']=now
        self._save_state()
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path=='/api/bot':
            with state.lock:
                body=state.bot.copy(); body['paper_only']=True; body['angel_enabled']=state.angel.enabled
            return self._send_json(body)
        if parsed.path=='/api/bot/test':
            with state.lock:
                candles=state._bot_underlying_candles(state.bot['underlying']); signal,conf,meta=strategy_signal(candles,state.bot['strategy']) if len(candles)>=25 else ('HOLD',0.0,{})
                state.bot['last_signal']=signal; state.bot['last_confidence']=conf; state.bot['last_reason']=json.dumps(meta,separators=(',',':')) if meta else 'Waiting for enough candles…'
        ...
        if parsed.path=='/api/backtest':
            ...
            try: days=max(1,min(30,int(p.get('days',['2'])[0]))); sl=max(0.1,min(20,float(p.get('sl',['0.6'])[0]))); tp=max(0.1,min(50,float(p.get('tp',['1.2'])[0])))
    ...
    def do_POST(self):
        ...
        with state.lock:
            if parsed.path=='/api/bot/control':
                ...
                if action=='START': state.bot['enabled']=True; state.bot['last_reason']='Bot running — waiting for a qualified signal…'
                elif action=='STOP': state.bot['enabled']=False; state.bot['last_reason']='Bot stopped'
                elif action=='CONFIG':
                    for k in ('strategy','underlying','instrument_mode'):
                        if k in payload and payload[k] in (['EMA_CROSS','RSI_MEAN_REVERT','VWAP_REVERT','BREAKOUT'] if k=='strategy' else ['NIFTY','SENSEX'] if k=='underlying' else ['INDEX','ATM_OPTIONS']): state.bot[k]=payload[k]
                    for k in ('qty','risk_per_trade','max_daily_loss','stop_loss_pct','target_pct'):
                        if k in payload:
                            try: state.bot[k]=max(0.01,float(payload[k]))
                            except: pass
                state._save_state(); self._send_json({'ok':True,'bot':state.bot,'paper_only':True}); return
