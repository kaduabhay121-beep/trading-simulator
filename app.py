...
class SimulationState:
    def __init__(self):
        ...
        self.live_chain_cache={}; self.market_cache={}; self.market_cache_ts=0; self.angel=AngelOneData(); self._init_history(); self._running=True
    ...
    def _live_indices(self):
        ...
        for inst, attr in [(ni,'nifty_spot'),(si,'sensex_spot')]:
            ...
            if ltp is None:
                ...
                if z and attr=='nifty_spot': self.prev_close=float(z.get('close',self.prev_close))
    ...
    def _tick_loop(self):
        ...
        while self._running:
            ...
            try:
                now=time.time()
                if self.angel.enabled and now-last>=0.2:
                    last=now
    ...
    def get_option_chain(self,symbol='NIFTY',expiry=None):
        ...
        cached=self.live_chain_cache.get(cache_key)
        if self.angel.enabled and cached and time.time()-cached[0] < 5.0:
            return cached[1]
        ...
        if self.angel.enabled:
            ...
            if exps:
                ...
                for s in strikes:
                    ...
                    rows.append(row)
                result={'expiries':exps,'selected_expiry':selected,'chain':rows}
                self.live_chain_cache[cache_key]=(time.time(),result)
                return result
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path=='/api/market':
            ...
            if state.market_cache.get('key')==cache_key and time.time()-state.market_cache_ts < 0.8:
